...
describe('IndexDefinition', function () {
    ...
    it('creates single column index definition', function () {
        ...
        expect($index->getColumns())->toBe(['email'])
            ->and($index->getIdentifier())->toBe('idx_email')
            ->and($index->isComposite())->toBeFalse();
    });

    it('creates composite index definition from array', function () {
        $index = new IndexDefinition(['user_id', 'created']);
        
        expect($index->getColumns())->toBe(['user_id', 'created'])
            ->and($index->getIdentifier())->toBe('idx_user_id_created')
            ->and($index->isComposite())->toBeTrue();
    });
    ...
    it('handles column names with underscores', function () {
        ...
        expect($index->getColumns())->toBe(['user_email'])
            ->and($index->getIdentifier())->toBe('idx_user_email');
    });

    it('handles complex column names', function () {
        $index = new IndexDefinition('account_verification_token');
        
        expect($index->getColumns())->toBe(['account_verification_token'])
            ->and($index->getIdentifier())->toBe('idx_account_verification_token');
    });

    it('handles three column composite index', function () {
        $index = new IndexDefinition(['user_id', 'post_id', 'created']);
        
        expect($index->getColumns())->toBe(['user_id', 'post_id', 'created'])
            ->and($index->getIdentifier())->toBe('idx_user_id_post_id_created')
            ->and($index->isComposite())->toBeTrue();
    });
    ...
    it('correctly identifies single column index as not composite', function () {
