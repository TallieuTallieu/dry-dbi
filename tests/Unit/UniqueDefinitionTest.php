...
describe('UniqueDefinition', function () {
    ...
    it('creates unique constraint definition', function () {
        ...
        expect($unique->getColumn())->toBe('email')
            ->and($unique->getIdentifier())->toBe('uq_email');
    });
    ...
    it('handles column names with underscores', function () {
        ...
        expect($unique->getColumn())->toBe('user_email')
            ->and($unique->getIdentifier())->toBe('uq_user_email');
    });

    it('handles complex column names', function () {
        $unique = new UniqueDefinition('account_verification_token');
        
        expect($unique->getColumn())->toBe('account_verification_token')
            ->and($unique->getIdentifier())->toBe('uq_account_verification_token');
    });
