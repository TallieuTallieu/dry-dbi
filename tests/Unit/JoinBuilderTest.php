...
describe('JoinBuilder', function () {
    it('creates LEFT JOIN', function () {
        ...
        expect($query)
            ->toContain('LEFT JOIN `users`')
            ->and($query)
            ->toContain('ON `posts`.`user_id` = `users`.`id`');
    });
    ...
    it('creates RIGHT JOIN', function () {
        ...
        expect($query)
            ->toContain('RIGHT JOIN `categories`')
            ->and($query)
            ->toContain('ON `posts`.`category_id` = `categories`.`id`');
    });
    ...
    it('creates INNER JOIN', function () {
        ...
        expect($query)
            ...
            ->and($query)
            ->toContain('ON `order_items`.`order_id` = `orders`.`id`');
    });
    ...
    it('handles table alias', function () {
        ...
        expect($query)
            ->toContain('LEFT JOIN `users` AS `u`')
            ->and($query)
            ->toContain('ON `posts`.`user_id` = `u`.`id`');
    });
    ...
    it('handles multiple ON conditions', function () {
        ...
        expect($query)
            ...
            ->and($query)
            ->toContain(
                'ON `posts`.`user_id` = `users`.`id` AND `posts`.`status` = `users`.`status`'
            );
    });
    ...
    it('throws exception for invalid join type', function () {
        ...
        expect(fn() => $join->setType('invalid'))->toThrow(
    ...
    it('handles complex table names', function () {
        ...
        expect($query)
            ->toContain('LEFT JOIN `user_profiles`')
            ->and($query)
            ->toContain('ON `users`.`id` = `user_profiles`.`user_id`');
    });
});
