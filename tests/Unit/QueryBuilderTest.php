...
describe('QueryBuilder - WHERE Clauses', function () {
    it('builds WHERE with single condition', function () {
        ...
        expect($qb->getQuery())
            ->toBe('SELECT `users`.* FROM `users` WHERE `users`.`status` = ?')
    ...
    it('builds WHERE with multiple AND conditions', function () {
        ...
        expect($qb->getQuery())
            ->toBe(
                'SELECT `users`.* FROM `users` WHERE `users`.`status` = ? AND `users`.`age` > ?'
            )
            ->and($qb->getParameters())
            ->toBe(['active', 18]);
    });
    ...
    it('builds WHERE with grouped conditions', function () {
        ...
        expect($qb->getQuery())->toContain(
            'WHERE `users`.`status` = ? AND  ( `users`.`role` = ? OR `users`.`role` = ? )'
        );
    });
    ...
    it('builds WHERE with OR grouped conditions', function () {
        ...
        expect($qb->getQuery())->toContain(
            'WHERE `users`.`status` = ? OR  ( `users`.`age` < ? AND `users`.`age` > ? )'
        );
    });
    ...
    it('handles different operators', function () {
        ...
        expect($qb->getQuery())
            ->toContain('WHERE `products`.`price` >= ?')
            ->and($qb->getQuery())
            ->toContain('AND `products`.`stock` <= ?')
            ->and($qb->getQuery())
            ->toContain('AND `products`.`status` != ?')
            ->and($qb->getParameters())
            ->toBe([100, 10, 'deleted']);
    });
});
...
describe('QueryBuilder - JOINS', function () {
    ...
    it('builds multiple JOINs', function () {
        ...
        expect($qb->getQuery())
            ->toContain('LEFT JOIN `users` ON `posts`.`user_id` = `users`.`id`')
            ->and($qb->getQuery())
            ->toContain(
                'LEFT JOIN `categories` ON `posts`.`category_id` = `categories`.`id`'
            );
...
describe('QueryBuilder - ORDER BY', function () {
    ...
    it('builds multiple ORDER BY clauses', function () {
        ...
        expect($qb->getQuery())->toBe(
            'SELECT `users`.* FROM `users` ORDER BY `users`.`status` ASC, `users`.`name` ASC'
        );
    });
...
describe('QueryBuilder - HAVING', function () {
    ...
    it('builds multiple HAVING conditions', function () {
        ...
        expect($qb->getQuery())
            ->toContain(
                'HAVING `orders`.`COUNT(*)` > ? AND `orders`.`SUM(total)` > ?'
            )
            ->and($qb->getParameters())
            ->toBe([5, 1000]);
    });
    ...
    it('builds HAVING with grouped conditions', function () {
        ...
        expect($qb->getQuery())->toContain(
            'HAVING  ( `orders`.`COUNT(*)` > ? OR `orders`.`COUNT(*)` < ? )'
        );
    });
});
...
describe('QueryBuilder - LIMIT and OFFSET', function () {
    it('builds LIMIT', function () {
        ...
        expect($qb->getQuery())
            ->toBe('SELECT `users`.* FROM `users` LIMIT ?')
            ->and($qb->getParameters())
            ->toBe([10]);
    });
    ...
    it('builds LIMIT with OFFSET', function () {
        ...
        expect($qb->getQuery())
            ->toBe('SELECT `users`.* FROM `users` LIMIT ? OFFSET ?')
            ->and($qb->getParameters())
            ->toBe([10, 20]);
    });
...
describe('QueryBuilder - Complex Queries', function () {
    it('builds complex SELECT with all clauses', function () {
        ...
        expect($query)
            ...
            ->toContain('LEFT JOIN `users` ON `posts`.`user_id` = `users`.`id`')
            ->and($query)
            ->toContain('WHERE `posts`.`status` = ? AND `posts`.`views` > ?')
            ->and($query)
            ->toContain('ORDER BY `posts`.`created_at` DESC')
            ->and($query)
            ->toContain('LIMIT ?')
            ->and($qb->getParameters())
            ->toBe(['published', 100, 10]);
    });
    ...
    it('builds query with GROUP BY and HAVING', function () {
        ...
        expect($query)
            ...
            ->and($query)
            ->toContain('GROUP BY `orders`.`user_id`')
            ->and($query)
            ->toContain('HAVING `orders`.`COUNT(*)` > ?')
            ->and($query)
            ->toContain('ORDER BY `orders`.`total_spent` DESC')
            ->and($query)
            ->toContain('LIMIT ?')
            ->and($qb->getParameters())
            ->toBe(['completed', 5, 20]);
...
describe('QueryBuilder - Fluent Interface', function () {
    ...
    it('allows method chaining for complex queries', function () {
        $qb = new QueryBuilder();
        $qb->table('posts')->select('id')->select('title');
        $qb->leftJoin('users')->on('posts.user_id', '=', 'users.id');
